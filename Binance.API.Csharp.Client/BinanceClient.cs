...
namespace Binance.API.Csharp.Client
{
    public class BinanceClient : BinanceClientAbstract, IBinanceClient
    {
        ...
        #region P2P Client
        public async Task<dynamic> Orders(int[] orderStatusList,int page=1,int rows=10)
        {
            var args = $"orderStatusList={string.Join(",",orderStatusList)}&page={page}&rows={rows}";
            var result = await _apiClient.CallAsync<dynamic>(ApiMethod.POST, EndPoints.Orders, true, args);
